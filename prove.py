...
def clear_mod_class(column):
    """
    Trasforma il dtype dei valori all'interno della colonna (object-->Int64).

    :param column: colonna "MOD_CLASS" con valori object
...
def get_components_size(df):
    ...
    :return: lista con all'interno il numero di nodi in ciascuna componente
    """
    components_size = []
    for row in range(len(df)):
        if pd.isna(df.at[row, "MOD_CLASS"]):
            # Ho trovato i voli isolati
            for i in range(df.at[row, "SIZE"]):
...
if __name__ == "__main__":
    ...
    louvain_df["MOD_CLASS"] = clear_mod_class(louvain_df["MOD_CLASS"])
    ...
    data = pd.DataFrame(mapped_flights.items(), columns=["flight", "cluster"])

    # Non mi occupo della size del cluster perchè la posso ottenere facilmente dal csv di Louvain
    data = data.groupby(["cluster"], dropna=False)["flight"].unique()
    data = pd.DataFrame({"CLUSTER": data.index, "FLIGHTS": data.values})

    with pd.option_context('display.max_rows', None, 'display.max_columns', None):
        data.to_csv(spectral_path, header=True, index=False, na_rep="None")
